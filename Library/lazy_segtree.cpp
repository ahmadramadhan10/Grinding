...
struct SegtreeLazy {
    ...
    T get(int v, int s, int e, int l, int r) {
        ...
        push(v, s, e);
        int mid = (s + e) >> 1;
        T p1 = get(v << 1, s, mid, l, min(r, mid)); // agar di luar range tidak di update
        T p2 = get(v << 1 | 1, mid + 1, e, max(l, mid + 1), r); // agar di luar range tidak di update
